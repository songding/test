...
namespace SCML
{
	...
	void Object::Init(TiXmlNode *node, Animator *animator)
	{
		sprite = NULL;
	...
	void Key::Init(TiXmlNode *node, Animator *animator)
	{
		TiXmlElement *element = node->ToElement();
		if (element)
		{
	...
	void Timeline::Init(TiXmlNode *node, Animator *animator)
	{
		int intValue;
	...
	void Animation::Init(TiXmlNode *node, Animator *animator)
	{
		int intValue;
		float floatValue;
		...
		if (element)
		{
			...
			for (TiXmlNode* lineNode = node->FirstChild(); lineNode; lineNode = lineNode->NextSibling())
			{
				...
				if (strcmp(tabLine, "mainline")==0)			// should be 1 mainline only
				{
	...
	Animator *Animator::create(const char *filename)
	{
	...
	CCSprite *Animator::getSprite(int folderId, int fileId)
	{
	...
	void Animator::setPosition(const cocos2d::CCPoint &pos)
	{
	...
	bool Animator::initWithFile(const char *filename)
	{
		mCurrEntity = 0;
